...
#handle O&M bottom conductivity which is comparable to lab value per daniel wisheropp
gfg_data2a <- gfg_data2 %>%
  mutate(analyte1a = case_when(data_owner =="2040" & analyte == "Field (Bottom) Specific Conductance"~ "Field Specific Conductance", TRUE ~ analyte))

unique(gfg_data2a$analyte1a)

#filter out all other methods not related to conductivity, move data under Field EC into Field SC because directly comparable
...
#as field sc for DEMP is done at same depth as discrete sample, bottom sc collected as well at different depth for permit compliance
gfg_data4 = filter(gfg_data3, analyte2 =="Field Specific Conductance"|analyte =="Specific Conductance" )

unique(gfg_data4$analyte2)


#need to separate out field SC from lab SC, manage case when values for both field bottom SC and field SC for single sample code, per Morgan Battey
#use field sc as this is done at same depth as discrete sample, bottom sc collected as well at different depth for permit compliance
...
#as field sc is done at same depth as discrete sample, bottom sc collected as well at different depth for permit compliance
...
#export R file 
write_xlsx(data_bound_rpd,'C:\\Users\\krein\\OneDrive - California Department of Water Resources\\Documents\\Projects\\Field Lab SC\\RPD calculation files from R\\RPD data from full data set_v3.xlsx')
...
>>>>>>> 4e0b28c64f9067a5272daa87a9317dbcf74ccb76
