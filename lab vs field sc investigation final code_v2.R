...
gfg_data3 = filter(gfg_data2, method =="Std Method 2510-B [1]*"| method =="EPA 120.1 (Field) [1]*"| method =="Std Method 2510-B (Field) [1]*"|method =="Std Method 2510 B (Filtered) [1]*") %>%
  mutate(analyte2 = case_when(analyte == "Field Electrical Conductance"~ "Field Specific Conductance",
                              TRUE ~ analyte))

unique(gfg_data3$analyte2)
                    
#filter out bottom field sc per morgan battey for DEMP
...
#export R file 
write_xlsx(data_bound_rpd,'C:\\Users\\krein\\OneDrive - California Department of Water Resources\\Documents\\Projects\\Field Lab SC\\RPD calculation files from R\\RPD data from full data set_v2.xlsx')
