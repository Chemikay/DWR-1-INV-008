...
#### Relative percent differences (RPDs) between these field and laboratory reported values are evaluated here. explore relative percent differences (RPD) between DWR field and Bryte Lab specific conductance (SC) measurements. Samples collected between March 2018 and February 2023 are evaluated here. Data were exported from the WDL database. See xyz script for data cleaning and reorganization.
...
df <- read_excel("~/Projects/Field Lab SC/RPD calculation files from R/RPD data from full data set_v3.xlsx")
...
#### Summary tables of sample counts and RPDS across period of concern ("1") and reference period ("2") and filtered for above and below 10000 uS/cm
...
table4 <- set_caption(table1, caption = "Table 4. Sample counts across period of concern and reference period by data owner ")
...
#export R file of focus data set

write_xlsx(df_1_focus,'C:\\Users\\krein\\OneDrive - California Department of Water Resources\\Documents\\Projects\\Field Lab SC\\focus_data_set_final_v2.xlsx')
...
#remove data not in time period 1 or 2, get a total of 952 samples
df_1_focus = filter(df_1, time_period_focus=="1" | time_period_focus=="2" )
...
#look at statistical significance between time periods 1 & 2 with major outlier removed, sample OM0123B0054
df_no_outlier1 = filter(df_1_focus, dwr_sample_code != "OM0123B0054")

#test for statistical significance in RPD between time periods 1 & 2 with sample OM0123B0054 removed
kruskal.test(rpd ~ time_period_focus, data = df_no_outlier1) #significant but close, p=0.04932 so did not impact much
...
write_xlsx(df_1_focus,'C:\\Users\\krein\\OneDrive - California Department of Water Resources\\Documents\\Projects\\Field Lab SC\\focus_data_set_final.xlsx')
